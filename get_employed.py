...
def get_emp(row):
	...
	econ_data = json_data["data"]["economicActivity"]["economicActiveType"]["countSeries"][0]["data"]
	return int(econ_data[0] * 1000 / (econ_data[0] + econ_data[1])) / 10

empdf = df.apply(get_emp, axis=1)
empdf.to_csv("employed.csv", header=None, index=None)
